...
'''def update_linear_schedule(optimizer, epoch, total_num_epochs, initial_lr, min_lr):
    """Decreases the learning rate linearly"""
    mul = max(0, float(total_num_epochs) - epoch)/float(total_num_epochs)
    lr = min_lr + (initial_lr - min_lr) * mul
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr'''
