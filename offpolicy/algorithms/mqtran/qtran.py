...
from offpolicy.algorithms.mqtran.algorithm.qtran_net import QtranV, QtranQBase, QtranQAlt
...
class QTran(Trainer):
    ...
    def train_policy_on_batch(self, batch, update_policy_id=None):
        ...
        for p_id in self.policy_ids:
            ...
            pol_nobs_batch = to_torch(nobs_batch[p_id])
            ...
            stacked_nobs_batch = torch.cat(list(pol_nobs_batch), dim=-2).to(**self.tpdv)
            ...
            if avail_act_batch[p_id] is not None:
                ...
                stacked_navail_act_batch = torch.cat(list(curr_navail_act_batch), dim=-2).to(**self.tpdv)
            else:
                stacked_avail_act_batch = None
                stacked_navail_act_batch = None

            # [num_agents, episode_length, episodes, dim]
            batch_size = pol_obs_batch.shape[1]

            individual_q_evals = policy.get_q_values(stacked_obs_batch)
            individual_q_targets = target_policy.get_q_values(stacked_nobs_batch)
            
            
            opt_onehot_eval,q_opt = policy.actions_from_q(individual_q_evals, available_actions=stacked_avail_act_batch)
            opt_onehot_target,_ = target_policy.actions_from_q(individual_q_targets, available_actions=stacked_navail_act_batch)

            q_evals = self.eval_joint_q(cent_obs_batch, stacked_act_batch.reshape(self.num_agents,batch_size,-1).transpose(0,1))
            q_targets = self.target_joint_q(cent_nobs_batch, torch.tensor(opt_onehot_target,dtype=torch.float32).reshape(self.num_agents,batch_size,-1).transpose(0,1).to(**self.tpdv))
            v = self.v(cent_obs_batch)
            
            q_sum_opt = q_opt.reshape(self.num_agents,batch_size,-1).transpose(0,1).sum(-1)
            joint_q_hat_opt = self.eval_joint_q(cent_obs_batch, torch.tensor(opt_onehot_eval,dtype=torch.float32).reshape(self.num_agents,batch_size,-1).transpose(0,1).to(**self.tpdv))

            q_individual = policy.q_values_from_actions(individual_q_evals, stacked_act_batch)
            q_sum_nopt = q_individual.reshape(self.num_agents,batch_size,-1).transpose(0,1).sum(dim=-1)
        ...
        q_evals_batch = torch.cat(q_evals_all, dim=-1).reshape(batch_size,-1)
        q_targets_batch = torch.cat(q_targets_all, dim=-1).reshape(batch_size,-1)
        v_batch = torch.cat(v_all, dim=-1).reshape(batch_size,-1)
        q_opt_batch = torch.cat(q_opt_all, dim=-1).reshape(batch_size,-1)
        joint_q_opt_batch = torch.cat(joint_q_opt_all, dim=-1).reshape(batch_size,-1)
        q_sum_nopt_batch = torch.cat(q_sum_nopt_all, dim=-1).reshape(batch_size,-1)
 
        # agents share reward
        rewards = to_torch(rew_batch[self.policy_ids[0]][0]).to(**self.tpdv)
        # form bad transition mask
        bad_transitions_mask = dones_env_batch
        #import pdb;pdb.set_trace()
        # L_td
