import numpy as np
import torch
from offpolicy.algorithms.mqtran.algorithm.agent_q_function import AgentQFunction
...
class QTranPolicy(RecurrentPolicy):
    ...
    def get_q_values(self, obs_batch, action_batch=None):
        ...
        q_batch = self.q_network(input_batch)
        #import pdb;pdb.set_trace()
        if action_batch is not None:
            action_batch = to_torch(action_batch).to(self.device)
            q_values = self.q_values_from_actions(q_batch, action_batch)
    ...
    def get_actions(self, obs, available_actions=None, t_env=None, explore=False):
        """See parent class."""
        q_values_out = self.get_q_values(obs)
        #import pdb;pdb.set_trace()
