...
class QtranQBase(nn.Module):
    def __init__(self, args, num_agents, cent_obs_dim, act_dim, device, multidiscrete_list=None):
        ...
        q_input = self.cent_obs_dim + self.act_dim
        self.q = nn.Sequential(nn.Linear(q_input, self.args.qtran_hidden_dim),
                               nn.ReLU(),
                               nn.Linear(self.args.qtran_hidden_dim, self.args.qtran_hidden_dim),
                               nn.ReLU(),
    ...
    def forward(self, state, actions):  # (episode_num, max_episode_len, n_agents, n_actions)
        episode_num, n_agents, _ = actions.shape
        hidden_actions = actions.reshape(-1, self.act_dim)
        hidden_actions_encoding = self.hidden_action_encoding(hidden_actions)
        hidden_actions_encoding = hidden_actions_encoding.reshape(episode_num, n_agents, -1) 
        hidden_actions_encoding = hidden_actions_encoding.sum(dim=-2)

        inputs = torch.cat([state.reshape(episode_num, -1), hidden_actions_encoding], dim=-1)
        q = self.q(inputs)
        return q
...
class QtranV(nn.Module):
    ...
    def forward(self, state):
        episode_num, _ = state.shape
        state = state.reshape(episode_num, -1)
        inputs = state
        v = self.v(inputs)
        return v