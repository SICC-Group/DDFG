...
class ObsRewardEncoder(nn.Module):
    def __init__(self, args,n_agents,act_dim,central_obs_dim,obs_dim, device):
        super(ObsRewardEncoder, self).__init__()
        self.args = args
        self.n_agents = n_agents
        self.n_actions = act_dim
        self.mixing_embed_dim = args.mixer_hidden_dim
        self.action_latent_dim = args.action_latent_dim
        self.device = device
        self.state_dim = central_obs_dim
    ...
    def forward(self):
        actions = th.Tensor(np.eye(self.n_actions)).to(self.device)
