...
class M_DFG:
    ...
    def train_policy_on_batch(self, batch, use_same_share_obs):
        ...
        for p_id in self.policy_ids:
            ...
            rewards = to_torch(rew_batch[p_id][0]).to(**self.tpdv) #[32,1]
            curr_obs_batch = to_torch(obs_batch[p_id]).transpose(0,1) #[32,8,48]
            curr_act_batch = to_torch(act_batch[p_id]).to(**self.tpdv).transpose(0,1) #[32,8,5]
            curr_nobs_batch = to_torch(nobs_batch[p_id]).transpose(0,1) #[32,8,48]
            ...
            curr_act_batch_ind = curr_act_batch.max(dim=-1)[1]
            #import pdb;pdb.set_trace()
            pol_all_q_out = policy.get_q_values(curr_obs_batch,curr_act_batch_ind.unsqueeze(dim=-1))
            #import pdb;pdb.set_trace()
            '''
            ...
            '''
            agent_qs.append(pol_all_q_out)

            with torch.no_grad():
                if self.args.use_double_q:
                    # actions come from live q; get the q values for the final nobs
                    greedy,_ = policy.get_actions(curr_nobs_batch,
                                                available_actions=stacked_navail_act_batch,
                                                t_env=None,explore=False)
                    curr_nact_batch_ind = torch.from_numpy(greedy).max(dim=-1)[1]
                    #import pdb;pdb.set_trace()
                    targ_pol_next_qs = target_policy.get_q_values(curr_nobs_batch, action_batch=curr_nact_batch_ind.unsqueeze(dim=-1))
                    '''
                    ...
                    greedy,_ = target_policy.get_actions(curr_nobs_batch,
                                                     available_actions=stacked_navail_act_batch,
                                                     t_env=None,explore=False)
                    curr_nact_batch_ind = torch.from_numpy(greedy).max(dim=-1)[1]
            ...
            agent_next_qs.append(targ_pol_next_qs)
        # combine the agent q value sequences to feed into mixer networks
        curr_Q_tot = torch.cat(agent_qs, dim=-1).unsqueeze(dim=-1)
        next_step_Q_tot = torch.cat(agent_next_qs, dim=-1).unsqueeze(dim=-1)
    ...
    def prep_training(self):
        """See parent class."""
        for p_id in self.policy_ids:
            self.policies[p_id].adj_network.train()
        for p_id in self.policy_ids:
