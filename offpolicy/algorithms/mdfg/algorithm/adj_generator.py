...
class Adj_Generator(nn.Module):
    def __init__(self, num_variable, num_factor ,highest_orders,device,temp = 10, temp_drop_frac = 0.999):
        super(Adj_Generator, self).__init__()
        #self.distribution = torch.distributions.bernoulli.Bernoulli(0.1)
        self.gen_matrix = Parameter(torch.rand(num_variable, num_factor, 2))
        #gen_matrix 为邻接矩阵的概率
        ...
        self.device = device
        self.highest_orders = highest_orders
        self.to(device)
    ...
    def sample(self, hard=False):
        ...
        out = gumbel_softmax_mdfg(self.logp, self.temperature, hard, self.num_factor,self.highest_orders,self.device)
