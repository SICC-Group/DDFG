...
class R_DDFG:
    def __init__(self, args, num_agents, policies, adj_network, policy_mapping_fn, device=torch.device("cuda:0"), episode_length=25,vdn=False):
        ...
        self.critic_fv_optimizer = torch.optim.Adam(params=self.critic_fv_parameters, lr=self.critic_lr, eps=self.opti_eps)
    ...
    def train_policy_on_batch(self, batch, use_same_share_obs=None):
        ...
        for p_id in self.policy_ids:
            ...
            with torch.no_grad():
                greedy,_,_,_ = policy.get_actions(obs_qtot, rnn_obs_qtot, curr_avail_act, None, False, adj_input_qtot, no_sequence,dones_qtot) 
                curr_nact_batch_ind = torch.from_numpy(greedy).max(dim=-1)[1].to(self.device)
                target_policy_qs = target_policy.get_q_values(obs_qtot, rnn_target_obs, curr_nact_batch_ind.unsqueeze(dim=-1), adj_input_qtot,no_sequence,dones_qtot)
        ...
        if self.use_vfunction: 
            ...
            #target_v_tot = torch.cat(target_fv, dim=-1).unsqueeze(-1)
            error_fv = (f_v_tot - curr_v_tot.detach()) * (1 - bad_transitions_mask)
            loss_fv = mse_loss(error_fv).sum() / ((1 - bad_transitions_mask).sum())
            #newloss = loss+loss_fv
            
            '''curr_v_tot = torch.cat(fv, dim=-1).unsqueeze(-1)
            next_step_v_tot = torch.cat(target_fv, dim=-1).unsqueeze(-1)
            v_tot_targets = rewards + (1 - dones_env_batch) * self.args.gamma * next_step_v_tot
            error_v = (curr_v_tot - v_tot_targets.detach()) * (1 - bad_transitions_mask)
            loss_v = mse_loss(error_v).sum() / ((1 - bad_transitions_mask).sum())
            newloss = loss+loss_v'''
