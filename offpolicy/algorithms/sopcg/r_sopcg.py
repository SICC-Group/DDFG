    ...
    def train_policy_on_batch(self, batch, use_same_share_obs=None):
        ...
        for p_id in self.policy_ids:
           ...
           # rnn_states_2 = policy.init_hidden(self.num_agents,batch_size)
            ...
            for t in range(step):      
                f, g, target_graphs_ = policy.get_rnn_batch(obs_1[t],batch_size,curr_avail_act_batch[t],no_sequence,select_graph=True)
                agent_outs = policy.constructor.compute_outputs(f, g, curr_avail_act_batch[t], target_graphs_)
                agent_outs_detach = agent_outs.detach()
                agent_outs_detach[curr_avail_act_batch[t] == 0] = -9999999
                cur_max_actions = agent_outs_detach.max(dim=-1, keepdim=True)[1]

                target_f, target_g = target_policy.get_rnn_batch(target_obs[t],batch_size,curr_avail_act_batch[t],no_sequence,select_graph=False)
                target_policy_qs.append(target_policy.constructor.compute_values_given_actions(target_f, target_g, cur_max_actions, target_graphs_))
            # Calculate estimated Q-Values for the current actions
                if t < step - 1:
                    graphs_used = policy.solver.solve_given_actions(f, g, curr_act_batch_ind[t], device=self.device)
                     # graphs_used = self.solver.graph_epsilon_greedy(graphs_used, self.args.graph_epsilon)
                    mac_values = policy.constructor.compute_values_given_actions(f, g, curr_act_batch_ind[t], graphs_used)
                    policy_qs.append(mac_values)
 
            qs.append(torch.stack(policy_qs, dim=0).reshape(step-1,batch_size,-1))
            target_qs.append(torch.cat(target_policy_qs[1:], dim=0).reshape(step-1,batch_size,-1))
