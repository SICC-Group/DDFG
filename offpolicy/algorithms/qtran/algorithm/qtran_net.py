...
class QtranQBase(nn.Module):
    def __init__(self, args, num_agents, cent_obs_dim, act_dim, device, multidiscrete_list=None):
        ...
        ae_input = self.hidden_size + self.act_dim
        ...
        q_input = self.cent_obs_dim + self.act_dim + self.hidden_size
        self.q = nn.Sequential(nn.Linear(q_input, self.args.qtran_hidden_dim),
                               nn.ReLU(),
                               nn.Linear(self.args.qtran_hidden_dim, self.args.qtran_hidden_dim),
                               nn.ReLU(),
    ...
    def forward(self, state, hidden_states, actions):  # (episode_num, max_episode_len, n_agents, n_actions)
        ...
        hidden_actions = hidden_actions.reshape(-1, self.hidden_size + self.act_dim)
...
class QtranV(nn.Module):
    def __init__(self, args, num_agents, cent_obs_dim, device, multidiscrete_list=None):
        ...
        v_input = self.cent_obs_dim + self.hidden_size
    ...
    def forward(self, state, hidden):
        ...
        hidden_encoding = self.hidden_encoding(hidden.reshape(-1, self.hidden_size))
