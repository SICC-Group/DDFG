...
class SMACRunner(RecRunner):
    ...
    def collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        while t < self.episode_length:
            ...
            if self.algorithm_name in self.adj_correlation:
                env_adj = adj.cpu().detach().numpy()[0] if self.algorithm_name in ["rddfg_cent_rw","rddfg_low"] else adj.cpu().detach().numpy()
                env_prob_adj = prob_adj.cpu().detach().numpy()[0] if self.algorithm_name in ["rddfg_cent_rw","rddfg_low"] else prob_adj.cpu().detach().numpy()
        ...
        if explore:
            ...
            ind = self.buffer.insert(self.num_envs,
                               ...
                               episode_adj,
                               episode_prob_adj)
            if self.algorithm_name  in ["rddfg_cent_rw","rddfg_low"] and self.total_env_steps >= self.adj_begin_step and (not warmup):
        ...
        # np.savetxt('results/StarCraft2/3s5z/adj.txt',episode_adj[p_id].reshape(-1,self.num_factor),fmt='%d')
        # np.savetxt('results/StarCraft2/3s5z/reward.txt',episode_rewards[p_id][:,0,0])
        # np.savetxt('results/StarCraft2/3s5z/acts.txt',np.argmax(episode_acts[p_id],axis=-1)[:,0],fmt='%d')
