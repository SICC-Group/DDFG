...
class MPERunner(MlpRunner):
    ...
    def shared_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        for step in range(self.episode_length):
            ...
            if step == self.episode_length-1:
                episode_dones = episode_dones.append(np.sum(dones!=0))
        
        for pid in self.policy_ids:
            if hasattr(self.policies[pid],'adj_network'):
                self.policies[pid].adj_network.drop_temperature()   
        #import pdb;pdb.set_trace() 
        average_episode_rewards = np.mean(np.sum(episode_rewards, axis=0)) / self.num_agents
        env_info['average_episode_rewards'] = average_episode_rewards
        #env_info['episode_dones'] = np.mean(episode_dones)
    ...
    def separated_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        average_episode_rewards = np.mean(np.sum(episode_rewards, axis=0)) / self.num_agents
        env_info['average_episode_rewards'] = average_episode_rewards
        #env_info['episode_dones'] = np.sum(dones!=0)
        #import pdb;pdb.set_trace()
        return env_info
