...
class MlpRunner(object):
    ...
    def __init__(self, config):
        ...
        elif self.algorithm_name == "mdfg":
            from offpolicy.algorithms.mdfg.algorithm.mDFGPolicy import M_DFGPolicy as Policy
            from offpolicy.algorithms.mdfg.mdfg import M_DFG as TrainAlgo
        else:
            raise NotImplementedError

        self.collecter = self.collect_rollout
        if self.algorithm_name == "mdfg":
            self.saver = self.save_q_mdfg
            self.restorer = self.restore_mdfg
    ...
    def save_q(self):
        ...
        if hasattr(self.trainer,'mixer') and not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)
            torch.save(self.trainer.mixer.state_dict(),
                   self.save_dir + '/mixer.pt')

    def save_q_mdfg(self):
        """Save all policies to the path specified by the config. Used for QMix and VDN."""
        for pid in self.policy_ids:
            p_save_path = self.save_dir + '/' + str(pid)
            if not os.path.exists(p_save_path):
                os.makedirs(p_save_path)
            policy_Q_1 = self.policies[pid].q_network[1]
            policy_Q_2 = self.policies[pid].q_network[2]
            policy_Q_3 = self.policies[pid].q_network[3]
            adj_Q = self.policies[pid].adj_network
            torch.save(policy_Q_1.state_dict(), p_save_path + '/q_network_1.pt')
            torch.save(policy_Q_2.state_dict(), p_save_path + '/q_network_2.pt')
            torch.save(policy_Q_3.state_dict(), p_save_path + '/q_network_3.pt')
            torch.save(adj_Q.state_dict(), p_save_path + '/adj_network.pt')
    ...
    def restore_mdfg(self):
        """Load policies policies from pretrained models specified by path in config. Used for QMix and VDN."""
        for pid in self.policy_ids:
            path = str(self.model_dir) + str(pid)
            print("load the pretrained model from {}".format(path))
            policy_q_state_dict_1 = torch.load(path + '/q_network_1.pt')
            policy_q_state_dict_2 = torch.load(path + '/q_network_2.pt')
            policy_q_state_dict_3 = torch.load(path + '/q_network_3.pt')
            adj_state_dict = torch.load(path + '/adj_network.pt')                  
            self.policies[pid].q_network[1].load_state_dict(policy_q_state_dict_1)
            self.policies[pid].q_network[2].load_state_dict(policy_q_state_dict_2)
            self.policies[pid].q_network[3].load_state_dict(policy_q_state_dict_3)
            self.policies[pid].adj_network.load_state_dict(adj_state_dict)
